...
SCENARIO("MortonCode2D_U64", "[morton][64bit][2d]")
{
    using namespace std;
    using namespace Onyx;

    using MortonT = MortonCode2D_U64;
...
SCENARIO("MortonCode3D_U64", "[morton][64bit][3d]")
{
    using namespace std;
    using namespace Onyx;

    using MortonT = MortonCode3D_U64;

    GIVEN("A random morton code")
    {
        using CoordinateT = MortonT::CoordinateT;
        constexpr CoordinateT MAX_COORD = 2097151;
        ...
        auto [initialX, initialY, initialZ] = GENERATE(
            ...
            make_tuple<CoordinateT, CoordinateT, CoordinateT>(2097151, 2097151, 2097151),
            take(100, randomNumberTriple<CoordinateT>(0, 2097150)));
        ...
        WHEN("Going right 1")
        {
            ...
            THEN("Y/Z stay the same X changes by 1")
            {
                ...
                REQUIRE(neighborX == ((initialX == MAX_COORD) ? 0 : static_cast<CoordinateT>(initialX + 1)));
                REQUIRE(neighborY == initialY);
        ...
        WHEN("Going forward 1")
        {
            ...
            THEN("X/Y stay the same Z changes by 1")
            {
                ...
                REQUIRE(neighborX == initialX);
                REQUIRE(neighborY == initialY);
                REQUIRE(neighborZ == ((initialZ == MAX_COORD) ? 0 : (initialZ + 1)));
            }
        }

...
TEMPLATE_TEST_CASE( "Vector3 Constructor",
...
TEMPLATE_TEST_CASE( "Vector3 Comparision",
                    "[Vector2][Comparision][template]",
                    Vector3f,
                    Vector3d,
                    Vector3s8,
                    Vector3s16,
...
TEMPLATE_TEST_CASE( "Vector3 Arithmetic",
                    "[Vector3][Arithmetic][template]",
                    Vector3f,
                    Vector3d,
                    Vector3s8,
                    Vector3s16,
...
TEMPLATE_TEST_CASE( "Vector3 Dot Product",
                    "[Vector3][DotProduct][template]",
                    Vector3f,
                    Vector3d,
                    Vector3s8,
                    Vector3s16,
    ...
    GIVEN( "2 Vectors a[1, 0, 0] and b[0, 1, 0]" ) {
        ...
        if constexpr ( std::is_floating_point_v< typename TestType::ScalarT > ||
                       std::is_signed_v< typename TestType::ScalarT > ) {
            THEN( "a . -a equals 1" ) {
                REQUIRE( a.Dot( -a ) == TestType::ScalarT( -1 ) );
            }
        }
...
TEMPLATE_TEST_CASE( "Vector3 Normalize",
    ...
    GIVEN( "a vector a[4, 3, 10]" ) {
        TestType a{ ScalarT( 4 ), ScalarT( 3 ), ScalarT( 10 ) };
        const float32 length = numericCast< float32 >( a.Length() );
...
TEMPLATE_TEST_CASE( "Vector3 Length",
                    "[Vector3][Length][template]",
                    Vector3f,
                    Vector3d,
                    Vector3s8,
                    Vector3s16,
...
TEMPLATE_TEST_CASE( "Vector3 Rotate",
                    "[Vector3][Rotate][template]",
                    Vector3f,
                    Vector3d,
                    Vector3s8,
                    Vector3s16,
    ...
    auto i = GENERATE( 30,
                       ...
                       -240,
                       -270,
                       take( 100, random( -180, 180 ) ) );

    const RotatedVectorScalarT angleRadians = static_cast< RotatedVectorScalarT >(
        i * std::numbers::pi_v< long double > / 180.0 );
    const RotatedVectorScalarT sinAlpha = std::sin( angleRadians );
    const RotatedVectorScalarT cosAlpha = std::cos( angleRadians );

    // X Axis rotation
    Matrix3< RotatedVectorScalarT > rotationMatrixPitch{ 1, 0, 0, 0, cosAlpha, -sinAlpha, 0, sinAlpha, cosAlpha };

    // Y Axis rotation
    Matrix3< RotatedVectorScalarT > rotationMatrixYaw{ cosAlpha, 0, sinAlpha, 0, 1, 0, -sinAlpha, 0, cosAlpha };

    // Z Axis rotation
    Matrix3< RotatedVectorScalarT > rotationMatrixRoll{ cosAlpha, -sinAlpha, 0, sinAlpha, cosAlpha, 0, 0, 0, 1 };

    constexpr float epsilon = []() {
        if constexpr ( sizeof( TestType::ScalarT ) <= sizeof( float32 ) )
            return 1e-5f;
        else
            return 1e-12f;
    }();
    ...
    GIVEN( "a vector [3, 5, 7]" ) {
        TestType a{ static_cast< TestType::ScalarT >( 3 ),
                    static_cast< TestType::ScalarT >( 5 ),
                    static_cast< TestType::ScalarT >( 7 ) };
        RotatedVectorT expected{ static_cast< RotatedVectorScalarT >( a[ 0 ] ),
                                 static_cast< RotatedVectorScalarT >( a[ 1 ] ),
                                 static_cast< RotatedVectorScalarT >( a[ 2 ] ) };
        // 30�
        {
...
TEMPLATE_TEST_CASE( "Vector3 Rotate To",
                    "[Vector3][Rotate][template]",
                    Vector3f,
                    Vector3d,
                    Vector3s16,
    ...
    auto values = GENERATE_COPY(
        take( 100,
              filter(
                  ...
                  chunk( 6, random< ScalarT >( static_cast< ScalarT >( -100 ), static_cast< ScalarT >( 100 ) ) ) ) ) );
    ...
    GIVEN( format::Format( "a vector [{}, {}, {}]", fromX, fromY, fromZ ) ) {
        ...
        WHEN( format::Format( "rotated to [{}, {}, {}]", toX, toY, toZ ) ) {
            ...
            const RotatedVectorT& expected = normalizedTo * numericCast< RotatedVectorScalarT >( a.Length() );
            THEN( format::Format( "a . a' is ~1.0" ) ) {
                float64 dot = rotated.Normalized().Dot( normalizedTo );
                REQUIRE( IsEqual< float64 >( dot, 1.0, epsilon ) );

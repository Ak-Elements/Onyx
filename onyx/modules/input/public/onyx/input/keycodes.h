...
static constexpr uint16_t Key_Count = enums::toIntegral( Key::Last ) - enums::toIntegral( Key::First );
...
inline bool IsModifierKey( Key key ) {
    ...
    return ( key == Key::Left_Ctrl ) || ( key == Key::Right_Ctrl ) || ( key == Key::Left_Shift ) ||
           ( key == Key::Right_Shift ) || ( key == Key::Left_Alt ) || ( key == Key::Right_Alt ) ||

...
{
    bool InputBindingAxis3D::DoUpdate(const InputSystem& inputSystem, Vector3f32& outInputValue)
    {
        outInputValue.X = inputSystem.GetAxisValue1D(0, m_AxisX);
        outInputValue.X = inputSystem.GetAxisValue1D(0, m_AxisY);
        outInputValue.Z = inputSystem.GetAxisValue1D(0, m_AxisZ);
    ...
    InputID InputBindingAxis3D::GetBoundInputForSlot(onyxU32 index) const
    {
        switch (index)
        {
    ...
    void InputBindingAxis3D::SetInputBindingSlot(onyxU32 index, InputID inputID)
    {
        switch (index)
        {
            case 0:
...
namespace Onyx
{
    bool Serialization<Input::InputBindingAxis3D>::Serialize(Serializer& serializer, const Input::InputBindingAxis3D& binding)
    {
    ...
    bool Serialization<Input::InputBindingAxis3D>::Deserialize(const Deserializer& deserializer, Input::InputBindingAxis3D& outBinding)
    {

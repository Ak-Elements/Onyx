...
class PrefabRegistry {
  public:
    Prefab CreateEntity() { return { m_PrefabRegistry.CreateEntity() }; }

    void AddComponent( EntityId entityId, uint32_t componentId, const file_system::JsonValue& json ) {
    ...
    void RegisterPrefab( uint32_t id, Prefab entity ) { m_Prefabs[ id ] = entity; }
    EntityRegistry& GetRegistry() { return m_PrefabRegistry; }

    Entity Instantiate( uint32_t prefabId, EntityRegistry& registry ) {
#if ONYX_IS_DEBUG
    ...
    Entity Instantiate( Prefab prefab, EntityRegistry& registry ) {
        ...
        for ( const auto&& componentStorageIt : m_PrefabRegistry.GetRegistry().storage() ) {
            if ( const auto& prefabComponentStorage = componentStorageIt.second;
                 prefabComponentStorage.contains( prefabEntityId ) ) {
  ...
  private:
    ...
    HashMap< uint32_t, Prefab > m_Prefabs;
};
} // namespace onyx::ecs

...
#include <onyx/graphicscore/shader/shaderincluder.h>
#include <onyx/graphicscore/shader/shaderpreprocessor.h>
...
namespace Onyx::Graphics::ShaderCompiler
{
	...
	namespace Glsl
	{
		...
		bool Compile(const GraphicsSystem& api, const FilePath& sourcePath, const String& preprocessedCode, ShaderStage stage, DynamicArray<onyxU32>& outByteCode)
		{
			...
			const shaderc::SpvCompilationResult module = compiler.CompileGlslToSpv(preprocessedCode.data(), preprocessedCode.length(), shaderKind, sourcePath.string().c_str(), shaderCOptions);

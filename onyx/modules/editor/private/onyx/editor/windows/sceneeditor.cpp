...
SceneEditorWindow::SceneEditorWindow() : m_WindowId( local_WindowId++ ) {
...
void SceneEditorWindow::OnOpen() {
    ...
    m_Dockspace = ui::Dockspace::Create( { { ui::DockSplitDirection::Right, 1.0f - entitiesPanelRatio, "", m_EntitiesPanelId },
...
void SceneEditorWindow::RenderSceneViewport() {
    ...
    if ( ImGui::Begin( format::Format( "{}{}", localization::editor::SceneEditor::SceneViewport, m_SceneViewPanelId ) ) ) {
        ...
        m_ViewportBounds.Extents = {
            static_cast< onyxS16 >( sceneTextureProperties.m_Size[ 0 ] ),
            static_cast< onyxS16 >( sceneTextureProperties.m_Size[ 1 ] ) }; // = Vector2s16{ static_cast<onyxS16>(ImGui::GetContentRegionAvail().x),
                                                                            // static_cast<onyxS16>(ImGui::GetContentRegionAvail().y) } - m_ViewportBounds.Position;
...
void SceneEditorWindow::RenderImGuizmo( const Vector2f32& viewportExtents ) {
    ...
    if ( selectedEntitesView.empty() == false ) {
        ...
        if ( m_HasSelectedEntity ) {
            game_core::TransformComponent& transformComponent = registry.GetComponent< game_core::TransformComponent >( selectedEntity );
            ...
            ImGuizmo::SetRect( 0, 0, viewportExtents[ 0 ], viewportExtents[ 1 ] );

            game_core::CameraComponent& editorCamera = registry.GetComponent< game_core::CameraComponent >( m_EditorCameraEntity );

            const Matrix4x4f32& projectionMatrix = editorCamera.Camera.GetProjectionMatrix();
...
void SceneEditorWindow::OnCameraMoveInput( const input_actions::InputActionEvent& inputActionContext ) {
    ...
    ecs::EntityRegistry& registry = m_Scene->GetRegistry();
    game_core::FreeCameraRuntimeComponent& cameraRuntimeComponent = registry.GetComponent< game_core::FreeCameraRuntimeComponent >( m_EditorCameraEntity );
...
void SceneEditorWindow::OnCameraSpeedInput( const input_actions::InputActionEvent& inputActionContext ) {
    ...
    ecs::EntityRegistry& registry = m_Scene->GetRegistry();
    game_core::FreeCameraControllerComponent& cameraControllerComponent = registry.GetComponent< game_core::FreeCameraControllerComponent >( m_EditorCameraEntity );
    game_core::FreeCameraRuntimeComponent& cameraRuntimeComponent = registry.GetComponent< game_core::FreeCameraRuntimeComponent >( m_EditorCameraEntity );
...
void SceneEditorWindow::OnSceneLoaded( const assets::AssetHandle< game_core::Scene >& sceneAsset ) {
    ...
    game_core::TransformComponent& transform = registry.AddComponent< game_core::TransformComponent >( m_EditorCameraEntity );
    ...
    const game_core::FreeCameraControllerComponent& freeCameraController = registry.AddComponent< game_core::FreeCameraControllerComponent >( m_EditorCameraEntity );
    game_core::FreeCameraRuntimeComponent& runtimeComponent = registry.AddComponent< game_core::FreeCameraRuntimeComponent >( m_EditorCameraEntity );

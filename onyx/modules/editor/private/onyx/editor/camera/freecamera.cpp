...
//     }
//
//     void FreeCamera::Update(uint64_t deltaTime)
//     {
//         if (ImGui::IsMouseDown(ImGuiMouseButton_Right))
//         {
//             constexpr float32 MAX_ROTATION_SPEED = 0.12f;
//
//             float32 dt = static_cast<float32>(deltaTime);
//
//             const float32 yawSign = m_UpDirection[1] < 0 ? -1.0f : 1.0f;
...
//
//             m_YawDelta += std::clamp(yawSign * m_InputRotation[0] * m_RotationSpeed, -MAX_ROTATION_SPEED,
//             MAX_ROTATION_SPEED); m_PitchDelta += std::clamp(m_InputRotation[1] * m_RotationSpeed,
...
//         transformMatrix[3] = (transformMatrix[0] * m_Position[0]) + (transformMatrix[1] * m_Position[1]) +
//         (transformMatrix[2] * m_Position[2]) + transformMatrix[3];
//
//         Matrix4<float32> rotationMatrix = GetOrientation().ToMatrix4();
//

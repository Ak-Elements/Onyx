...
bool ColorPicker( StringView label, VectorT& outColor, ImGuiColorEditFlags flags, const ImVec4* ref_col ) {
    ...
    ImGui::ColorConvertHSVtoRGB( H, ScalarT( 1 ), ScalarT( 1 ), hue_color_f.x, hue_color_f.y, hue_color_f.z );
    ...
    if( flags & ImGuiColorEditFlags_PickerHueWheel ) {
        ...
        for( int n = 0; n < 6; n++ ) {
            ...
            ImGui::ShadeVertsLinearColorGradientKeepAlpha( draw_list,
                                                           vert_start_idx,
                                                           vert_end_idx,
                                                           gradient_p0,
                                                           gradient_p1,
        ...
        ImGui::RenderFrameBorder( ImVec2( bar0_pos_x, picker_pos.y ),
...

    {
        ...
        if( ( flags & ( ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV ) ) != 0 &&
            ( flags & ImGuiColorEditFlags_NoInputs ) == 0 ) {
            {
                ...
                const uint32_t
                    colorX = isDisplayingRGB
                                 ? VectorControl::BACKGROUND_COLOR_X_COMPONENT
                                 : VectorControl::BACKGROUND_COLOR_W_COMPONENT; // ColorConvertHSVtoBGR(floatVector.X,
                                                                                // 1.0f, 1.0f);
                const uint32_t
                    colorY = isDisplayingRGB
                                 ? VectorControl::BACKGROUND_COLOR_Y_COMPONENT
                                 : VectorControl::BACKGROUND_COLOR_W_COMPONENT; // ColorConvertHSVtoBGR(floatVector.X,
                                                                                // floatVector.Y, 1.0f);
                const uint32_t
                    colorZ = isDisplayingRGB
                                 ? VectorControl::BACKGROUND_COLOR_Z_COMPONENT
                                 : VectorControl::BACKGROUND_COLOR_W_COMPONENT; // ColorConvertHSVtoBGR(floatVector.X,
                                                                                // 1.0f, floatVector.Z);
                ...
                if( flags & ImGuiColorEditFlags_Float ) {
                    ...
                    value_changed |= VectorControl::VectorComponentInput( labelX,
                                                                          floatVector.X,
                                                                          ImGuiDataType_Float,
                                                                          componentInputSize,
                                                                          colorX );
                    value_changed |= VectorControl::VectorComponentInput( labelY,
                                                                          floatVector.Y,
                                                                          ImGuiDataType_Float,
                                                                          componentInputSize,
                                                                          colorY );
                    value_changed |= VectorControl::VectorComponentInput( labelZ,
                                                                          floatVector.Z,
                                                                          ImGuiDataType_Float,
                                                                          componentInputSize,
                                                                          colorZ );

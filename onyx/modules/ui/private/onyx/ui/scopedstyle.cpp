...
namespace onyx::ui {
ScopedImGuiStyle::ScopedImGuiStyle( int32_t styleVar, float32 value ) {
    ImGui::PushStyleVar( styleVar, value );
}

ScopedImGuiStyle::ScopedImGuiStyle( int32_t styleVar, const Vector2f32& value ) {
    ImGui::PushStyleVar( styleVar, { value[ 0 ], value[ 1 ] } );
}

ScopedImGuiStyle::ScopedImGuiStyle( int32_t styleVar, const ImVec2& value ) {
    ImGui::PushStyleVar( styleVar, value );
}

ScopedImGuiStyle::ScopedImGuiStyle( std::initializer_list< std::pair< int32_t, StyleVarValue > > styleVars ) {
    m_StyleVarsCount = static_cast< int32_t >( styleVars.size() );
    for ( const auto& [ styleEnum, value ] : styleVars ) {
        ...
        if ( std::holds_alternative< float32 >( value ) ) {
            ImGui::PushStyleVar( styleEnum, std::get< float32 >( value ) );

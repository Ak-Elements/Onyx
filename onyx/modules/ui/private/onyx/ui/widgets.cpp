...
bool ContextMenuHeader( StringView label, ImGuiTreeNodeFlags flags ) {
    ...
    if ( isOpen ) {
        ...
        float32 margin = ( ( header_max.x - header_min.x ) - label_size.x );
...
void DrawInfoIcon( ImDrawList* draw_list, ImVec2 offset, float32 radius, uint32_t color ) {
    ...
    float32 line_height = radius * 0.5f;
    float32 line_width = radius * 0.2f;
...
void DrawSearchIcon( ImDrawList* draw_list, ImVec2 offset, float32 radius, uint32_t color ) {
    ...
    float32 lens_radius = radius * 0.5f; // Adjust the size of the circle

    // Calculate the offset for top-left adjustment
    float32 offset_x = radius * 0.1f; // Adjust this value to change the offset amount horizontally
    float32 offset_y = radius * 0.1f; // Adjust this value to change the offset amount vertically
    ...
    float32 handle_length = radius * 0.3f;
    float32 handle_width = radius * 0.2f;
...
bool DrawCloseButton( ImDrawList* draw_list, ImVec2 offset, float32 size, uint32_t color ) {
    ImVec2 cursorPos = ImGui::GetCursorScreenPos();
    ...
    float32 line_thickness = size * 0.2f; // Thickness of the lines
    float32 line_length = size * 1.2f;    // Length of the lines
...
void DrawFolderIcon( ImDrawList* draw_list,
    ...
    float32 tab_height = verticalSize * 0.2f; // Height of the top section
    float32 tab_width = size * 0.3f;          // Width of the top section
    float32 open_angle = verticalSize * 0.1f; // Opening effect
    ...
    // Draw folder body
    draw_list->AddRectFilled( top_left + ImVec2( 0, open_angle ),
                              bottom_right,
                              color_folder,
                              rounding,
    ...
    ImVec2 lid_points[ 5 ] = { top_left, // Top-left of the lid
...
void DrawPlusIcon( ImDrawList* draw_list, ImVec2 offset, float32 size, uint32_t color ) {
    ImVec2 cursorPos = ImGui::GetCursorScreenPos();
...
void DrawMinusIcon( ImDrawList* draw_list, ImVec2 offset, float32 size, uint32_t color ) {
    ImVec2 cursorPos = ImGui::GetCursorScreenPos();
    ImVec2 center = cursorPos - offset;

    // Define the size of the "X" lines
    float32 line_thickness = size * 0.2f; // Thickness of the lines
    float32 line_length = size * 1.0f;    // Length of the lines
...
void DrawXIcon( ImDrawList* draw_list, ImVec2 offset, float32 size, uint32_t color ) {
    ImVec2 cursorPos = ImGui::GetCursorScreenPos();
    ImVec2 center = cursorPos - offset;
...
void DrawDivisionIcon( ImDrawList* draw_list, ImVec2 offset, float32 size, uint32_t color ) {
    ImVec2 cursorPos = ImGui::GetCursorScreenPos();
    ImVec2 center = cursorPos - offset;

    float32 line_thickness = size * 0.2f;
    float32 line_length = size * 1.0f;
    float32 dot_radius = size * 0.1f;

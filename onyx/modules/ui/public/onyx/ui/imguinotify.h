...
enum class ImGuiToastPhase : uint8_t { Queued, FadeIn, Wait, FadeOut, Expired, COUNT };
...
enum class ImGuiToastDirection : uint8_t { Up, Down, COUNT };
...
struct ImGuiNotifySettings {
    ...
    int32_t DefaultAutoDismissInMilliseconds = 3000;   // Auto dismiss after X ms (default, applied only of no data
                                                       // provided in constructors)
    ...
    bool RenderOnScreen = false; // Set to true to render on screen instead of inside main window - Needs ImGui
                                 // Multiviewport
    ...
    ImGuiToastDirection
        Direction = ImGuiToastDirection::Down; // Direction of the toast notifications if more than 1 are shown
...
class ImGuiToast {
  private:
    ...
    std::function< bool() >
        m_OnPress = nullptr; // A lambda variable, which will be executed when button in notification is pressed, return
                             // true if the notification should be removed
    std::function< bool() >
        m_CustomContent = nullptr; // A lambda which can be used to render custom imgui content within the notification,
  ...
  public:
     ...
     */
    inline void SetOnButtonPress( const std::function< bool() >& onButtonPress ) { m_OnPress = onButtonPress; }
  ...
  public:
     ...
     */
    inline ImGuiToastType GetType() { return m_Type; };
     ...
     */
    inline std::string_view GetContent() { return m_Content; };
     ...
     */
    inline uint64_t GetElapsedTime() { return onyx::Time::GetCurrentMilliseconds() - m_CreationTime; }
    ...
    inline void Update( ImGuiNotifySettings& settings ) {
        ...
        if( elapsed > settings.FadeOutAnimationTimeInMilliseconds + m_DismissTime +
                          settings.FadeOutAnimationTimeInMilliseconds ) {
    ...
    inline float GetFadePercent( ImGuiNotifySettings& settings ) {
        ...
        } else if( m_State == ImGuiToastPhase::FadeOut ) {
            return ( 1.f -
                     ( ( (float)elapsed - (float)settings.FadeOutAnimationTimeInMilliseconds - (float)m_DismissTime ) /
                       settings.FadeOutAnimationTimeInMilliseconds ) ) *
                   settings.ToastAlpha;
        }
     ...
     */
    inline ImGuiWindowFlags GetWindowFlags() { return m_Flags; }

    /**
     * @return The function, which is executed on left click on the notification.
     */
    inline const std::function< bool() >& GetOnPress() { return m_OnPress; }
  ...
  public:
    ...
    ImGuiToast( ImGuiToastType type,
                ...
                const std::function< bool() >& onPress )
 ...
 */
inline ImGuiToast& InsertNotification( ImGuiToastType type,
                                       int32_t dismissTime = NotificationSettings.DefaultAutoDismissInMilliseconds ) {
...
/**
 * @brief Constructor for creating a new ImGuiToast object with a specified type, dismiss time, title format, content
 * format and a button.
 *
...
inline void RenderNotifications() {
    ...
    for( int32_t i = 0; i < static_cast< int32_t >( Notifications.size() ); ++i ) {
        ...
        if( ( NotificationSettings.AutoDismissOnlyVisibleNotifications ) &&
            ( currentToast->GetState() == ImGuiToastPhase::Queued ) ) {
        ...
        const float opacity = currentToast->GetFadePercent(
            NotificationSettings ); // Get opacity based of the current phase
        ...
        SetNextWindowSizeConstraints( ImVec2( NotificationSettings.MinSize.x, NotificationSettings.MinSize.y ),
                                      ImVec2( NotificationSettings.MaxSize.x, NotificationSettings.MaxSize.y ) );
        ...
        // Here we render the toast content
        {
            ...
            if( title.empty() == false ) {
                ...
                TextEx(
                    defaultTitle.data() ); // Render default title text (ImGuiToastType_Success -> "Success", etc...)
                wasTitleRendered = true;
            }

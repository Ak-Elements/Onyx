...
const uint8_t Nodes[ 8 ] = { 0, 1, 2, 3, 4, 5, 6, 7 };

template < typename OctreeNodeT >
void Dualgrid< OctreeNodeT >::GenerateDualgrid( const OctreeNodeT& root,
    ...
    if ( root.IsSubdivided() == false ) {
        ...
        const auto GetNodePosition = []( const OctreeNodeT& node,
                                         const Vector3f32& halfExtentScale = Vector3f32( 0.0f ) ) {
            Vector3f32 position = node.GetData()->Position;
            position += halfExtentScale * node.GetData()->HalfExtent;
            return position;
        };

        AddBorderDualCell( GetNodePosition( root, CORNER_0 ),
                           GetNodePosition( root, CENTER_BACK_BOTTOM ),
                           GetNodePosition( root, FACE_BOTTOM ),
                           GetNodePosition( root, CENTER_LEFT_BOTTOM ),
                           GetNodePosition( root, CENTER_BACK_LEFT ),
                           GetNodePosition( root, FACE_BACK ),
                           GetNodePosition( root ),
                           GetNodePosition( root, FACE_LEFT ) );
        AddBorderDualCell( GetNodePosition( root, CENTER_BACK_BOTTOM ),
                           GetNodePosition( root, CORNER_1 ),
                           GetNodePosition( root, CENTER_RIGHT_BOTTOM ),
                           GetNodePosition( root, FACE_BOTTOM ),
                           GetNodePosition( root, FACE_BACK ),
                           GetNodePosition( root, CENTER_BACK_RIGHT ),
                           GetNodePosition( root, FACE_RIGHT ),
                           GetNodePosition( root ) );
        AddBorderDualCell( GetNodePosition( root, FACE_BOTTOM ),
                           GetNodePosition( root, CENTER_RIGHT_BOTTOM ),
                           GetNodePosition( root, CORNER_2 ),
                           GetNodePosition( root, CENTER_FRONT_BOTTOM ),
                           GetNodePosition( root ),
                           GetNodePosition( root, FACE_RIGHT ),
                           GetNodePosition( root, CENTER_FRONT_RIGHT ),
                           GetNodePosition( root, FACE_FRONT ) );
        AddBorderDualCell( GetNodePosition( root, CENTER_LEFT_BOTTOM ),
                           GetNodePosition( root, FACE_BOTTOM ),
                           GetNodePosition( root, CENTER_FRONT_BOTTOM ),
                           GetNodePosition( root, CORNER_3 ),
                           GetNodePosition( root, FACE_LEFT ),
                           GetNodePosition( root ),
                           GetNodePosition( root, FACE_FRONT ),
                           GetNodePosition( root, CENTER_FRONT_LEFT ) );

        AddBorderDualCell( GetNodePosition( root, CENTER_BACK_LEFT ),
                           GetNodePosition( root, FACE_BACK ),
                           GetNodePosition( root ),
                           GetNodePosition( root, FACE_LEFT ),
                           GetNodePosition( root, CORNER_4 ),
                           GetNodePosition( root, CENTER_BACK_TOP ),
                           GetNodePosition( root, FACE_TOP ),
                           GetNodePosition( root, CENTER_LEFT_TOP ) );
        AddBorderDualCell( GetNodePosition( root, FACE_BACK ),
                           GetNodePosition( root, CENTER_BACK_RIGHT ),
                           GetNodePosition( root, FACE_RIGHT ),
                           GetNodePosition( root ),
                           GetNodePosition( root, CENTER_BACK_TOP ),
                           GetNodePosition( root, CORNER_5 ),
                           GetNodePosition( root, CENTER_RIGHT_TOP ),
                           GetNodePosition( root, FACE_TOP ) );
        AddBorderDualCell( GetNodePosition( root ),
                           GetNodePosition( root, FACE_RIGHT ),
                           GetNodePosition( root, CENTER_FRONT_RIGHT ),
                           GetNodePosition( root, FACE_FRONT ),
                           GetNodePosition( root, FACE_TOP ),
                           GetNodePosition( root, CENTER_RIGHT_TOP ),
                           GetNodePosition( root, CORNER_6 ),
                           GetNodePosition( root, CENTER_FRONT_TOP ) );
        AddBorderDualCell( GetNodePosition( root, FACE_LEFT ),
                           GetNodePosition( root ),
                           GetNodePosition( root, FACE_FRONT ),
                           GetNodePosition( root, CENTER_FRONT_LEFT ),
                           GetNodePosition( root, CENTER_LEFT_TOP ),
...
template < typename OctreeNodeT >
void Dualgrid< OctreeNodeT >::EdgeProcX( const OctreeNodeT& node0,
...
template < typename OctreeNodeT >
void Dualgrid< OctreeNodeT >::EdgeProcY( const OctreeNodeT& node0,
                                         const OctreeNodeT& node1,
                                         const OctreeNodeT& node2,
                                         const OctreeNodeT& node3 ) {
    const bool n0Subdivided = node0.IsSubdivided();
...
template < typename OctreeNodeT >
void Dualgrid< OctreeNodeT >::EdgeProcZ( const OctreeNodeT& node0,
                                         const OctreeNodeT& node1,
                                         const OctreeNodeT& node2,
                                         const OctreeNodeT& node3 ) {
    const bool n0Subdivided = node0.IsSubdivided();
...
template < typename OctreeNodeT >
void Dualgrid< OctreeNodeT >::VertexProc( const OctreeNodeT& node0,
                                          const OctreeNodeT& node1,
                                          const OctreeNodeT& node2,
                                          const OctreeNodeT& node3,
...
template < typename OctreeNodeT >
void Dualgrid< OctreeNodeT >::AddDualCell( const OctreeNodeT& node0,
                                           const OctreeNodeT& node1,
                                           const OctreeNodeT& node2,
                                           const OctreeNodeT& node3,
                                           const OctreeNodeT& node4,
    ...
    //{
    //     m_MarchingSquaresIsoSurface->AddMarchingSquaresTriangles(corners, values,
    //     IsoSurface<float32>::MS_CORNERS_BACK);
    // }
    // if (corners[2][2] == to[2]/* && corners[2].z != mTotalTo.z*/)
    //{
    //     m_MarchingSquaresIsoSurface->AddMarchingSquaresTriangles(corners, values,
    //     IsoSurface<float32>::MS_CORNERS_FRONT);
    // }
    // if (corners[0][0] == from[0]/* && corners[0].x != mTotalFrom.x*/)
    //{
    //     m_MarchingSquaresIsoSurface->AddMarchingSquaresTriangles(corners, values,
    //     IsoSurface<float32>::MS_CORNERS_LEFT);
    // }
    // if (corners[1][0] == to[0]/* && corners[1].x != mTotalTo.x*/)
    //{
    //     m_MarchingSquaresIsoSurface->AddMarchingSquaresTriangles(corners, values,
    //     IsoSurface<float32>::MS_CORNERS_RIGHT);
    // }
    // if (corners[5][1] == to[1]/* && corners[5].y != mTotalTo.y*/)
    //{
    //     m_MarchingSquaresIsoSurface->AddMarchingSquaresTriangles(corners, values,
    //     IsoSurface<float32>::MS_CORNERS_TOP);
    // }
    // if (corners[0][1] == from[1]/* && corners[0].y != mTotalFrom.y*/)
    //{
    //     m_MarchingSquaresIsoSurface->AddMarchingSquaresTriangles(corners, values,
    //     IsoSurface<float32>::MS_CORNERS_BOTTOM);
    // }
}

template < typename OctreeNodeT >
void Dualgrid< OctreeNodeT >::CreateBorderCell( const OctreeNodeT& node0,
                                                const OctreeNodeT& node1,
                                                const OctreeNodeT& node2,
                                                const OctreeNodeT& node3,
                                                const OctreeNodeT& node4,
                                                ...
                                                const OctreeNodeT& node6,
                                                const OctreeNodeT& node7 ) {
    const auto IsFlagSet = []( const OctreeNodeT& node, VolumeOctreeNodeMetaData flag ) -> bool {
        return static_cast< uint8_t >( node.GetData()->MetaData & flag ) != 0;
    };
    ...
    if ( IsFlagSet( node0, VolumeOctreeNodeMetaData::BorderBack ) &&
        ...
        if ( IsFlagSet( node4, VolumeOctreeNodeMetaData::BorderTop ) &&
            ...
            AddBorderDualCell( GetNodePosition( node4, FACE_BACK ),
                               ...
                               GetNodePosition( node5 ),
                               GetNodePosition( node4 ),
                               GetNodePosition( node4, CENTER_BACK_TOP ),
                               GetNodePosition( node5, CENTER_BACK_TOP ),
            ...
            if ( IsFlagSet( node4, VolumeOctreeNodeMetaData::BorderLeft ) ) {
                AddBorderDualCell( GetNodePosition( node4, CENTER_BACK_LEFT ),
                                   ...
                                   GetNodePosition( node4, CORNER_4 ),
                                   GetNodePosition( node4, CENTER_BACK_TOP ),
                                   GetNodePosition( node4, FACE_TOP ),
            ...
            if ( IsFlagSet( node5, VolumeOctreeNodeMetaData::BorderRight ) ) {
                AddBorderDualCell( GetNodePosition( node5, FACE_BACK ),
                                   ...
                                   GetNodePosition( node5 ),
                                   GetNodePosition( node5, CENTER_BACK_TOP ),
                                   GetNodePosition( node5, CORNER_5 ),
                                   GetNodePosition( node5, CENTER_RIGHT_TOP ),
                                   GetNodePosition( node5, FACE_TOP ) );
            }
        }
        if ( IsFlagSet( node0, VolumeOctreeNodeMetaData::BorderBottom ) &&
            ...
            if ( IsFlagSet( node0, VolumeOctreeNodeMetaData::BorderLeft ) ) {
                AddBorderDualCell( GetNodePosition( node0, CORNER_0 ),
                                   ...
                                   GetNodePosition( node0, FACE_BOTTOM ),
                                   GetNodePosition( node0, CENTER_LEFT_BOTTOM ),
                                   GetNodePosition( node0, CENTER_BACK_LEFT ),
                                   GetNodePosition( node0, FACE_BACK ),
                                   GetNodePosition( node0 ),
        ...
        if ( IsFlagSet( node6, VolumeOctreeNodeMetaData::BorderTop ) &&
            ...
            if ( IsFlagSet( node7, VolumeOctreeNodeMetaData::BorderLeft ) ) {
                AddBorderDualCell( GetNodePosition( node7, FACE_LEFT ),
                                   GetNodePosition( node7 ),
                                   GetNodePosition( node7, FACE_FRONT ),
                                   GetNodePosition( node7, CENTER_FRONT_LEFT ),
            ...
            if ( IsFlagSet( node6, VolumeOctreeNodeMetaData::BorderRight ) ) {
                AddBorderDualCell( GetNodePosition( node6 ),
                                   ...
                                   GetNodePosition( node6, CORNER_6 ),
                                   GetNodePosition( node6, CENTER_FRONT_TOP ) );
            }
        }
        if ( IsFlagSet( node3, VolumeOctreeNodeMetaData::BorderBottom ) &&
            ...
            if ( IsFlagSet( node3, VolumeOctreeNodeMetaData::BorderLeft ) ) {
                AddBorderDualCell( GetNodePosition( node3, CENTER_LEFT_BOTTOM ),
                                   ...
                                   GetNodePosition( node3, CENTER_FRONT_BOTTOM ),
                                   GetNodePosition( node3, CORNER_3 ),
    ...
    if ( IsFlagSet( node0, VolumeOctreeNodeMetaData::BorderLeft ) &&
        ...
        if ( IsFlagSet( node0, VolumeOctreeNodeMetaData::BorderBottom ) &&
            ...
            AddBorderDualCell( GetNodePosition( node0, CENTER_LEFT_BOTTOM ),
                               GetNodePosition( node0, FACE_BOTTOM ),
                               GetNodePosition( node3, FACE_BOTTOM ),
                               GetNodePosition( node3, CENTER_LEFT_BOTTOM ),
                               GetNodePosition( node0, FACE_LEFT ),
                               GetNodePosition( node0 ),
                               GetNodePosition( node3 ),
                               GetNodePosition( node3, FACE_LEFT ) );
        ...
        if ( IsFlagSet( node0, VolumeOctreeNodeMetaData::BorderBack ) &&
            ...
            AddBorderDualCell( GetNodePosition( node0, CENTER_BACK_LEFT ),
                               ...
                               GetNodePosition( node0 ),
                               GetNodePosition( node0, FACE_LEFT ),
                               GetNodePosition( node4, CENTER_BACK_LEFT ),
                               GetNodePosition( node4, FACE_BACK ),
                               GetNodePosition( node4 ),
                               GetNodePosition( node4, FACE_LEFT ) );
        }
...
template < typename OctreeNodeT >
void Dualgrid< OctreeNodeT >::AddBorderDualCell( const Vector3f32& position0,

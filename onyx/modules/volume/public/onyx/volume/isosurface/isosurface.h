...
enum class IsoSurfaceMethod { DMC, DMC_WITH_CMS_ERROR_METRIC, CMS, None };
...
class IsoSurface {
  public:
    ...
    static constexpr uint8_t MS_CORNERS_FRONT[ 4 ] = { 7, 6, 2, 3 };

    /// To call Marching Squares with a cube on its back.
    static constexpr uint8_t MS_CORNERS_BACK[ 4 ] = { 5, 4, 0, 1 };

    /// To call Marching Squares with a cube on its left.
    static constexpr uint8_t MS_CORNERS_LEFT[ 4 ] = { 4, 7, 3, 0 };

    /// To call Marching Squares with a cube on its right.
    static constexpr uint8_t MS_CORNERS_RIGHT[ 4 ] = { 6, 5, 1, 2 };

    /// To call Marching Squares with a cube on its top.
    static constexpr uint8_t MS_CORNERS_TOP[ 4 ] = { 4, 5, 6, 7 };

    /// To call Marching Squares with a cube on its bottom.
    static constexpr uint8_t MS_CORNERS_BOTTOM[ 4 ] = { 3, 2, 1, 0 };

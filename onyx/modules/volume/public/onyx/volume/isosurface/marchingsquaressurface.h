...
class MarchingSquaresSurface : public IsoSurface< T > {
  ...
  public:
    ...
    void addMarchingSquaresTriangles( const Vector3< T > corners[],
        ...
        for( size_t i = 0; i < 4; ++i ) {
            ...
            if( values[ i ][ 3 ] >= Super::IsoLevel ) {
                squareIndex |= 1 << i;
            }
        }

        // Don't generate triangles if we are completely inside and farPlane enough away from the surface
        if( squareIndex == 15 && values[ 0 ][ 3 ] >= m_maxDistance && values[ 1 ][ 3 ] >= m_maxDistance &&
            values[ 2 ][ 3 ] >= m_maxDistance && values[ 3 ][ 3 ] >= m_maxDistance ) {
            return;
        }
        ...
        if( edge & 1 ) {
            intersectionPoints[ 1 ] = Super::calculateSurfacePositionAndNormal( corners[ indices[ 0 ] ],
                                                                                corners[ indices[ 1 ] ],
                                                                                values[ 0 ],
                                                                                values[ 1 ],
        ...
        if( edge & 2 ) {
            intersectionPoints[ 3 ] = Super::calculateSurfacePositionAndNormal( corners[ indices[ 1 ] ],
                                                                                corners[ indices[ 2 ] ],
                                                                                values[ 1 ],
                                                                                values[ 2 ],
        ...
        if( edge & 4 ) {
            intersectionPoints[ 5 ] = Super::calculateSurfacePositionAndNormal( corners[ indices[ 2 ] ],
                                                                                corners[ indices[ 3 ] ],
                                                                                values[ 2 ],
                                                                                values[ 3 ],
        ...
        if( edge & 8 ) {
            intersectionPoints[ 7 ] = Super::calculateSurfacePositionAndNormal( corners[ indices[ 3 ] ],
                                                                                corners[ indices[ 0 ] ],
                                                                                values[ 3 ],
                                                                                values[ 0 ],
        ...
        for( unsigned char i = 0; MarchingSquares::Triangles[ squareIndex ][ i ] != -1; i += 3 ) {
            m_meshBuilder->addTriangle( intersectionPoints[ MarchingSquares::Triangles[ squareIndex ][ i ] ],
                                        intersectionNormals[ MarchingSquares::Triangles[ squareIndex ][ i ] ],
                                        intersectionNormals[ MarchingSquares::Triangles[ squareIndex ][ i ] ],
                                        intersectionPoints[ MarchingSquares::Triangles[ squareIndex ][ i + 1 ] ],
                                        intersectionNormals[ MarchingSquares::Triangles[ squareIndex ][ i + 1 ] ],
                                        intersectionNormals[ MarchingSquares::Triangles[ squareIndex ][ i + 1 ] ],
                                        intersectionPoints[ MarchingSquares::Triangles[ squareIndex ][ i + 2 ] ],
                                        intersectionNormals[ MarchingSquares::Triangles[ squareIndex ][ i + 2 ] ],
                                        intersectionNormals[ MarchingSquares::Triangles[ squareIndex ][ i + 2 ] ] );
        }
    }

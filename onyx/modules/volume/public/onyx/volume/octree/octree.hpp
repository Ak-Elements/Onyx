...
template < typename NodeDataContainer, typename KeyType >
auto Octree< NodeDataContainer, KeyType >::FindLeafInternal( const OctreeKeyT& key,
                                                             const OctreeNodeT& startNode,
                                                             KeyType startNodeLevel,
                                                             KeyType& outLevel ) const -> const OctreeNodeT& {
    const OctreeNodeT* currentNode = &startNode;
    uint8_t childBranchIndex = 0;
...
auto Octree< NodeDataContainer, KeyType >::FindLeafInternal( const OctreeKeyT& key,
                                                             ...
                                                             KeyType startNodeLevel,
                                                             KeyType& outLevel ) -> OctreeNodeT& {
    OctreeNodeT* currentNode = &startNode;
    uint8_t childBranchIndex = 0;
    uint32_t levelMask = 1 << startNodeLevel;
    // TODO protected against overflow!

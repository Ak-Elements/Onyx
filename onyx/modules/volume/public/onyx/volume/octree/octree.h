...
template < typename NodeDataContainer, typename KeyType = uint32_t >
class Octree {
  public:
  ...
  private:
    static constexpr uint8_t ROOT_LEVEL = OctreeKeyT::MaxDepth - 1;
  ...
  public:
    ...
    DepthFirstIterator begin( uint8_t maxDepth = 0 ) { return DepthFirstIterator( this, maxDepth ); };
    ...
    DepthFirstLeafNodeIterator leaf_begin( uint8_t maxDepth = 0 ) {
        return DepthFirstLeafNodeIterator( this, maxDepth );
    };
    ...
    typedef const OctreeBreadthFirstIterator< Octree< NodeDataContainer >, OctreeNodeT >
        ConstBreadthFirstLeafNodeIterator;
    BreadthFirstLeafNodeIterator breadth_begin( uint8_t maxDepth = 0 ) {
    ...
    const OctreeNodeT& FindLeaf( KeyType x,
                                 KeyType y,
                                 KeyType z,
                                 const OctreeNodeT& startNode,
                                 KeyType startNodeLevel,
                                 KeyType& outLevel ) const {
    ...
    const OctreeNodeT& FindLeaf( const OctreeKeyT& key,
                                 const OctreeNodeT& startNode,
                                 KeyType startNodeLevel,
                                 KeyType& outLevel ) const {
    ...
    OctreeNodeT& FindLeaf( KeyType x,
                           KeyType y,
                           KeyType z,
                           const OctreeNodeT& startNode,
                           KeyType startNodeLevel,
    ...
    OctreeNodeT& FindLeaf( const OctreeKeyT& key,
                           const OctreeNodeT& startNode,
                           KeyType startNodeLevel,
                           KeyType& outLevel ) {
        return FindLeafInternal( key, startNode, startNodeLevel, outLevel );
  ...
  private:
    const OctreeNodeT& FindLeafInternal( const OctreeKeyT& key,
                                         const OctreeNodeT& startNode,
                                         KeyType startNodeLevel,
                                         KeyType& outLevel ) const;
    OctreeNodeT& FindLeafInternal( const OctreeKeyT& key,
                                   OctreeNodeT& startNode,
                                   KeyType startNodeLevel,
                                   KeyType& outLevel );

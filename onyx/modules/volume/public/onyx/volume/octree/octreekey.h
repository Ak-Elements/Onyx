...
class OctreeKey {
  ...
  public:
    ...
    static constexpr uint8_t MaxDepth = sizeof( KeyType ) * 8;
    ...
    bool operator==( const OctreeKeyT& other ) const {
        return ( m_Key[ 0 ] == other.m_Key[ 0 ] ) && ( m_Key[ 1 ] == other.m_Key[ 1 ] ) &&
               ( m_Key[ 2 ] == other.m_Key[ 2 ] );
    }

    bool operator!=( const OctreeKeyT& other ) const { return operator==( other ) == false; }

    bool operator<=( const OctreeKey& other ) const {
        return ( other.m_Key[ 0 ] >= m_Key[ 0 ] ) && ( other.m_Key[ 1 ] >= m_Key[ 1 ] ) &&
               ( other.m_Key[ 2 ] >= m_Key[ 2 ] );
    }
    ...
    void TraverseDown( KeyType depth, uint8_t childIndex ) {
    ...
    uint8_t GetChildBranchBit( KeyType level ) const {
    ...
    Vector3f32 GetNodeRealPosition( const float32 rootSize, KeyType depth ) const {
        ...
        while ( depth < ( MaxDepth - 1 ) ) {
            ...
            float32 cellSize = rootSize / ( 1 << ( MaxDepth - depth ) );
            if ( isX )
                position[ 0 ] += cellSize;
            else
                position[ 0 ] -= cellSize;

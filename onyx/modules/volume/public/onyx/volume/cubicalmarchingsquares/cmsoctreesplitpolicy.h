...
class CMSOctreeSplitPolicy : public OctreeSplitPolicy< Scalar > {
  ...
  public:
    CMSOctreeSplitPolicy( uint8_t maxOctreeLevel,
                          Scalar octreeRootSize,
                          Scalar maxGeometricError,
                          Scalar complexSurfaceThreshold,
    ...
    bool ShouldSplit( VolumeChunk::VolumeChunkOctree::OctreeNodeT& node,
        ...
        const Vector3< Scalar > corners[ 8 ] = {
            corner0,
            Vector3< Scalar >( nodeWorldPosition[ 0 ] + halfExtent,
                               nodeWorldPosition[ 1 ] - halfExtent,
                               nodeWorldPosition[ 2 ] - halfExtent ), // corner1
            Vector3< Scalar >( nodeWorldPosition[ 0 ] + halfExtent,
                               nodeWorldPosition[ 1 ] - halfExtent,
                               nodeWorldPosition[ 2 ] + halfExtent ), // corner2
            Vector3< Scalar >( nodeWorldPosition[ 0 ] - halfExtent,
                               nodeWorldPosition[ 1 ] - halfExtent,
                               nodeWorldPosition[ 2 ] + halfExtent ), // corner3
            Vector3< Scalar >( nodeWorldPosition[ 0 ] - halfExtent,
                               nodeWorldPosition[ 1 ] + halfExtent,
                               nodeWorldPosition[ 2 ] - halfExtent ), // corner4
            Vector3< Scalar >( nodeWorldPosition[ 0 ] + halfExtent,
                               nodeWorldPosition[ 1 ] + halfExtent,
                               nodeWorldPosition[ 2 ] - halfExtent ), // corner5
            corner6,
            Vector3< Scalar >( nodeWorldPosition[ 0 ] - halfExtent,
                               nodeWorldPosition[ 1 ] + halfExtent,
                               nodeWorldPosition[ 2 ] + halfExtent ), // corner7
  ...
  private:
    bool ExceedsGeometricError( const Vector3< Scalar > ( &corners )[ 8 ],
                                const Vector4< Scalar > ( &cornerValues )[ 8 ],
                                const Vector3< Scalar >& nodeCenter ) {
        using std::abs;
        using std::numeric_limits;

        // Error metric of http://www.andrew.cmu.edu/user/jessicaz/publication/meshing/
        const float32 f000 = cornerValues[ 0 ][ 3 ];
        const float32 f001 = cornerValues[ 1 ][ 3 ];
        const float32 f010 = cornerValues[ 2 ][ 3 ];
        const float32 f011 = cornerValues[ 3 ][ 3 ];
        const float32 f100 = cornerValues[ 4 ][ 3 ];
        const float32 f101 = cornerValues[ 5 ][ 3 ];
        const float32 f110 = cornerValues[ 6 ][ 3 ];
        const float32 f111 = cornerValues[ 7 ][ 3 ];
        ...
        Vector3< Scalar > positions[ 19 ][ 2 ] = { { centerBackBottom, Vector3< Scalar >( 0.5f, 0.0f, 0.0f ) },
    ...
    Scalar Interpolate( Scalar f000,
                        Scalar f001,
                        Scalar f010,
                        Scalar f011,
                        Scalar f100,
        ...
        return oneMinZ * ( f000 * oneMinXoneMinY + f100 * xOneMinY + f010 * oneMinX * position[ 1 ] ) +
               position[ 2 ] * ( f001 * oneMinXoneMinY + f101 * xOneMinY + f011 * oneMinX * position[ 1 ] ) +
               position[ 0 ] * position[ 1 ] * ( f110 * oneMinZ + f111 * position[ 2 ] );
    }

#pragma once

#include <onyx/geometry/vector3.h>
#include <onyx/geometry/vector4.h>

#include <onyx/volume/source/noise/simplexnoised.h>
#include <onyx/volume/source/volumebase.h>

namespace onyx::volume {
class SimplexNoiseSource : public VolumeBase {
  public:
    SimplexNoiseSource() = default;
    SimplexNoiseSource( uint32_t octaves,
                        float32 frequency,
                        float32 amplitude,
                        float32 lacunarity,
                        float32 persistence )
        : m_Octaves( octaves )
        , m_Frequency( frequency )
        , m_Amplitude( amplitude )
        , m_Lacunarity( lacunarity )
        , m_Gain( persistence ) {}

    enum class Dimension { Dimension_1D, Dimension_2D, Dimension_3D };

    Dimension GetDimension() const { return m_Dimension; }
    void SetDimension( Dimension value ) { m_Dimension = value; }

    uint32_t GetOctaves() const { return m_Octaves; }
    void SetOctaves( uint32_t value ) { m_Octaves = value; }

    float32 GetFrequency() const { return m_Frequency; }
    void SetFrequency( float32 value ) { m_Frequency = value; }

    float32 GetAmplitude() const { return m_Amplitude; }
    void SetAmplitude( float32 value ) { m_Amplitude = value; }

    float32 GetLacunarity() const { return m_Lacunarity; }
    void SetLacunarity( float32 value ) { m_Lacunarity = value; }

    float32 GetGain() const { return m_Gain; }
    void SetGain( float32 value ) { m_Gain = value; }

    float32 GetScale() const { return m_Scale; }
    void SetScale( float32 scale ) { m_Scale = scale; }

    void SetFrequencies( std::vector< float32 >& frequencies ) {
        for ( uint32_t i = 0; i < frequencies.size(); ++i ) {
            if ( i >= m_PredefinedOctaves.size() ) {
                m_PredefinedOctaves.emplace_back();
            }

            m_PredefinedOctaves[ i ].m_Frequency = frequencies[ i ];
        }
    }

    void SetAmplitudes( std::vector< float32 >& amplitudes ) {
        for ( uint32_t i = 0; i < amplitudes.size(); ++i ) {
            if ( i >= m_PredefinedOctaves.size() ) {
                m_PredefinedOctaves.emplace_back();
            }

            m_PredefinedOctaves[ i ].m_Amplitude = amplitudes[ i ];
        }
    }

    Vector3f32 GetAnalyticalNormal( const Vector3f32& position ) const {
        Vector3f32 deriative; // optimize away
        GetInternalValue( position, deriative, true );

        Vector3f32 analyticalNormal( -deriative[ 0 ], 1.0f - deriative[ 1 ], -deriative[ 2 ] );
        analyticalNormal.Normalize();

        return analyticalNormal;
    }

    virtual Vector4f32 GetValueAndGradient( const Vector3f32& position ) const override {
        /*
        const float32 offset = 0.001f;// m_Noise.GetGradientOffset();
        float32 val = GetInternalValue(position, deriative);
        Vector3f otherNormal;
        Vector3f centralDiffNormal(-(GetInternalValue(Vector3
        f(position[0] + offset, position[1], position[2]), otherNormal) - GetInternalValue(Vector3f(position[0] -
        offset, position[1], position[2]), otherNormal)),
            -(GetInternalValue(Vector3f(position[0], position[1] + offset, position[2]), otherNormal) -
        GetInternalValue(Vector3f(position[0], position[1] - offset, position[2]), otherNormal)),
            -(GetInternalValue(Vector3f(position[0], position[1], position[2] + offset), otherNormal) -
        GetInternalValue(Vector3f(position[0], position[1], position[2] - offset), otherNormal)));

        centralDiffNormal.Normalize();

        return Vector4f(centralDiffNormal[0], centralDiffNormal[1], centralDiffNormal[2], val);*/

        Vector3f32 deriative; // optimize away
        float32 val = GetInternalValue( position, deriative, true );

        Vector3f32 analyticalNormal( -deriative[ 0 ], 1.0f - deriative[ 1 ], -deriative[ 2 ] );
        analyticalNormal.Normalize();

        return Vector4f32( analyticalNormal, val );
    }

    virtual float32 GetValue( const Vector3f32& position ) const override {
        Vector3f32 normal;
        return GetInternalValue( position, normal );
    }

  private:
    inline float32 GetInternalValue( const Vector3f32& position, Vector3f32& normal, bool getGradient = false ) const {
        float32 distance = 0;
        Vector3f32 planeNormal( 0, 1, 0 );

        float32 value = distance - static_cast< float32 >( planeNormal.Dot( position ) );
        float32 noise = GetNoiseVal( position, normal, getGradient );

        return value + noise;
    }

    float32 GetNoiseVal( const Vector3f32& position, Vector3f32& gradient, bool getGradient = false ) const {
        float32 frequency = m_Frequency;
        float32 amplitude = 0.5f;

        Vector3f32 pos = position * m_Scale;

        float32 noiseValue = 0.0f;

        // Vector3f deriviateSum;
        uint32_t octaves = m_Octaves;
        if ( m_PredefinedOctaves.empty() == false ) {
            octaves = static_cast< uint32_t >( m_PredefinedOctaves.size() );
        }

        Vector3f32 noiseD;
        Vector3f32 deriative;
        for ( size_t i = 0; i < octaves; i++ ) {
            // float32 noiseVal = 0.0f;

            if ( m_PredefinedOctaves.empty() == false ) {
                frequency = m_PredefinedOctaves[ i ].m_Frequency;
                amplitude = m_PredefinedOctaves[ i ].m_Amplitude;
            }

            switch ( m_Dimension ) {
            case Dimension::Dimension_1D:
                SimplexNoiseD::sdnoise1( pos[ 0 ] * frequency, noiseD );
                break;
            case Dimension::Dimension_2D: {
                Vector3f32 tempPos( pos.X * frequency, pos.Z * frequency, 0 );
                SimplexNoiseD::sdnoise2( tempPos, noiseD, getGradient );

                deriative.X = noiseD.Y;
                deriative.Z = noiseD.Z;
                break;
            }
            case Dimension::Dimension_3D: {
                Vector4f32 noise3;
                SimplexNoiseD::sdnoise3( pos, noise3 );
                // noiseVal = noise3[0];
                deriative = Vector3f32( noise3[ 1 ], noise3[ 2 ], noise3[ 3 ] );
                break;
            }
            default: {
                deriative = Vector3f32();
                break;
            }
            }

            noiseValue += ( amplitude * noiseD.X );
            if ( getGradient ) {
                gradient += deriative * ( frequency * amplitude );
            }

            if ( m_PredefinedOctaves.empty() ) {
                frequency *= m_Lacunarity;
                amplitude *= m_Gain;
            }
        }

        const float32 amplitudeScaled = m_Amplitude * m_Scale;
        Vector4f32 output( noiseValue * m_Amplitude,
                           gradient.X * amplitudeScaled,
                           gradient.Y * amplitudeScaled,
                           gradient.Z * amplitudeScaled );
        gradient.X = output[ 1 ];
        gradient.Y = output[ 2 ];
        gradient.Z = output[ 3 ];

        return output[ 0 ];
    }

  private:
    struct Octave {
        float32 m_Frequency = 1.0f;
        float32 m_Amplitude = 0.5f;
    };

    float32 m_Scale = 0.0f;
    uint32_t m_Octaves = 0;
    float32 m_Frequency = 0.0f;
    float32 m_Amplitude = 0.0f;
    float32 m_Lacunarity = 0.0f;
    float32 m_Gain = 0.0f;

    std::vector< Octave > m_PredefinedOctaves;

    Dimension m_Dimension = Dimension::Dimension_2D;
};
} // namespace onyx::volume
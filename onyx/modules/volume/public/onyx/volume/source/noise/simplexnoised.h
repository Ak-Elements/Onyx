 ...
 */
static constexpr unsigned char perm[ 512 ] = {
 ...
 */
static constexpr float32 grad2lut[ 8 ][ 2 ] = { { -1.0f, -1.0f },
                                                { 1.0f, 0.0f },
                                                { -1.0f, 0.0f },
                                                { 1.0f, 1.0f },
                                                { -1.0f, 1.0f },
...
inline void grad2( int hash, float32* gx, float32* gy ) {
    int h = hash & 7;
    *gx = grad2lut[ h ][ 0 ];
    *gy = grad2lut[ h ][ 1 ];
    return;
...
inline void sdnoise2( const Vector3f32& pos, Vector3f32& output, bool calculateGradient ) {
    float32 n0, n1, n2;                   /* Noise contributions from the three simplex corners */
    float32 gx0, gy0, gx1, gy1, gx2, gy2; /* Gradients at simplex corners */
    float32 t0, t1, t2, x1, x2, y1, y2;
    float32 t20, t40, t21, t41, t22, t42;

    /* Skew the input space to determine which simplex cell we're in */
    float32 s = ( pos.X + pos.Y ) * F2; /* Hairy factor for 2D */
    // float32 xs = pos.X + s;
    // float32 ys = pos.Y + s;
    ...
    float32 Y0 = j - t;
    float32 x0 = pos.X - X0; /* The x,y distances from the cell origin */
    float32 y0 = pos.Y - Y0;
    ...
    float32& outX = output.X;
    float32& outY = output.Y;
    float32& outZ = output.Z;
    ...
    if ( calculateGradient ) {
         ...
         */
        float32 temp = t20 * t0 * ( gx0 * x0 + gy0 * y0 );
        outY = temp * x0;
        outZ = temp * y0;

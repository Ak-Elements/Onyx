...
void SdfSphereVolumeShaderGraphNode::doGenerateShader( const node_graph::ExecutionContext& context,
    ...
    generator.addImport( "includes.volume.csg.sphere" );
    ...
    String uvVariableName = format::format( "sphereUv_{:x}", getId().get() );
    ...
    //     format::format( "float2 {} = {}.getUv( worldPosition );\n", uvVariableName, sphereVariableName ) );

...
{
    IntersectVolumeShaderGraphNode::IntersectVolumeShaderGraphNode()
    {
        m_DebugName = "Intersect";
    }

    void IntersectVolumeShaderGraphNode::OnUpdate(NodeGraph::ExecutionContext& /*context*/) const
    {
    }

    void IntersectVolumeShaderGraphNode::DoGenerateShader(const NodeGraph::ExecutionContext& context, Graphics::ShaderGenerator& generator) const
    {
        ...
        generator.AppendCode(Format::Format("vec3 {} = {}.xyz;\n", outPin0VariableName, intersectVariableName));
        generator.AppendCode(Format::Format("float {} = {}.w;\n", outPin1VariableName, intersectVariableName));
    }


    NodeGraph::PinBase* IntersectVolumeShaderGraphNode::GetInputPin(onyxU32 index)
    {
        switch (index)
        {
    ...
    const NodeGraph::PinBase* IntersectVolumeShaderGraphNode::GetInputPin(onyxU32 index) const
    {
        switch (index)
        {
        case 0: return &m_GradientSource0InPin;
    ...
    NodeGraph::PinBase* IntersectVolumeShaderGraphNode::GetOutputPin(onyxU32 index)
    {
    ...
    const NodeGraph::PinBase* IntersectVolumeShaderGraphNode::GetOutputPin(onyxU32 index) const
    {
        ONYX_ASSERT(index < GetOutputPinCount(), "Invalid pin index");
        if (index == 0)
            return &m_GradientOutPin;
...
#if ONYX_IS_EDITOR
    StringView IntersectVolumeShaderGraphNode::GetPinName(StringId32 pinId) const
    {
        switch (pinId)
        {

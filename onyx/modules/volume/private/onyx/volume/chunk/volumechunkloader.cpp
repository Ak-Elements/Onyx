...
void VolumeChunkLoader::RequestLoad(
    const VolumeChunckLoadRequestData& reqData,
    InplaceFunction< void( const VolumeChunckLoadRequestData& ), 64 >&& finishedCallback ) {
    Cancel();
    ClearCanceledTasks();

    m_ActiveLoadRequest = makeUnique< VolumeChunkLoadRequest >(
        reqData,
        std::forward< InplaceFunction< void( const VolumeChunckLoadRequestData& ), 64 > >( finishedCallback ) );
...
void VolumeChunkLoader::ClearCanceledTasks() {
    std::erase_if( m_PendingCanceledTasks,
                   []( auto& pendingCanceledTask ) { return pendingCanceledTask->IsFinished(); } );
}
} // namespace onyx::volume

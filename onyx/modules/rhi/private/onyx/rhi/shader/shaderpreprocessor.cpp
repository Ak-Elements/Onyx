    ...
    while ( reader.isEof() == false ) {
        ...
        if ( shaderStage == ShaderStage::Invalid ) {
            ...
            if ( isShaderStage ) {
                ...
                PreprocessedShader& shader = m_PreprocessedShaderStages[ static_cast< uint8_t >( shaderStage ) ];
            ...
            if ( ParseStageCode( reader,
                                 shaderStage,
                                 m_PreprocessedShaderStages[ static_cast< uint8_t >( shaderStage ) ] ) == false ) {
...
bool ShaderPreprocessor::IsShaderStage( StringStream& reader, ShaderStage& outStage ) const {
    for ( uint8_t i = static_cast< uint8_t >( ShaderStage::Vertex ); i < static_cast< uint8_t >( ShaderStage::Count );
          ++i ) {
...
bool ShaderPreprocessor::ParseStageCode( StringStream& reader,
                                         ShaderStage stage,
    ...
    while ( reader.isEof() == false ) {
        ...
        if ( hasShaderStageScopeBegun == false ) {
            ...
            if ( currentLine[ firstCharIndex ] != '{' ) {
                ONYX_LOG_ERROR( "Expected {{ got {} in shader stage {}.",
                                currentLine[ firstCharIndex ],
        ...
        bracesCount += static_cast< int32_t >( std::ranges::count( currentLine, '{' ) );
        bracesCount -= static_cast< int32_t >( std::ranges::count( currentLine, '}' ) );
        if ( bracesCount < 0 ) {
        ...
        if ( bracesCount == 1 ) {
            ...
            if ( ( ( customFormatStart != StringView::npos ) && ( customFormatEnd == StringView::npos ) ) ||
                ...
                ONYX_LOG_ERROR( "Missing '{{}}' bracket in shader stage {}.",
                                ( customFormatStart == StringView::npos ) ? '<' : '>',
                                enums::toString( stage ) );
                return false;
            }

            if ( ( customFormatStart != StringView::npos ) && ( customFormatEnd != StringView::npos ) ) {
                TextureFormat attachmentFormat = enums::fromString< TextureFormat >(
                    currentLine.substr( customFormatStart + 1, ( customFormatEnd - customFormatStart ) - 1 ) );
                ...
                StringView attachmentName = currentLine.substr( attachmentNameStart,
                                                                ( attachmentNameEnd - attachmentNameStart ) );

                StringId32 attachmentId( attachmentName );
                ONYX_ASSERT( preprocessShader.m_Formats.contains( attachmentId ) == false,
                             "Format for this attachment is already defined." );
                preprocessShader.m_Formats[ attachmentId ] = attachmentFormat;

                preprocessShader.m_Code += currentLine.substr( 0, customFormatStart );

                preprocessShader.m_Code += currentLine.substr( customFormatEnd + 1,
                                                               ( attachmentNameEnd - customFormatEnd ) );
...
void ShaderPreprocessor::Serialize( Stream& outStream, const ShaderPreprocessor& preprocessor ) {
    outStream.write< uint64_t >( preprocessor.m_PreprocessedShaderStages.size() );

    for ( const PreprocessedShader& stage : preprocessor.m_PreprocessedShaderStages ) {
        outStream.write< uint64_t >( stage.m_Code.length() );

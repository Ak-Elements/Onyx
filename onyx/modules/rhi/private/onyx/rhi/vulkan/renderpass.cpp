...
VkAttachmentDescription2 CreateAttachmentDescription(
    const onyx::rhi::RenderPassSettings::Attachment& attachmentSettings,
    ...
    for ( uint8_t subPassIndex = 0; subPassIndex < subPassCount; ++subPassIndex ) {
        ...
        for ( uint8_t attachmentIndex = 0; attachmentIndex < attachmentCount; ++attachmentIndex ) {
            ...
            if ( ( subpass.m_AttachmentAccesses[ attachmentIndex ] ==
                ...
                depthStencilAttachment.layout = GetLayoutFromAttachmentRole(
                    subpass.m_AttachmentAccesses[ attachmentIndex ] );
        ...
        subpassDescription2.inputAttachmentCount = static_cast< uint32_t >( inputAttachments.size() );
        subpassDescription2.pInputAttachments = inputAttachments.empty() ? nullptr : inputAttachments.data();
        subpassDescription2.colorAttachmentCount = static_cast< uint32_t >( colorAttachments.size() );

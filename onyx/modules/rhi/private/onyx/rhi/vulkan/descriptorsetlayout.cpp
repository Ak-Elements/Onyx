...
DescriptorSetLayout::DescriptorSetLayout( const Device& device,
    ...
    : m_Device( device )
    , m_Set(
          set ){ VK_CHECK_RESULT( vkCreateDescriptorSetLayout( device.GetHandle(), &createInfo, nullptr, &m_Layout ) ) }
    ...
    for ( auto& [ binding, storageBuffer ] : descriptorSet.StorageBuffers ) {
        ...
        layoutBinding.pImmutableSamplers = nullptr;
        layoutBinding.binding = binding;
        ONYX_ASSERT( descriptorSet.UniformBuffers.find( binding ) == descriptorSet.UniformBuffers.end(),
    ...
    for ( auto& [ binding, imageSampler ] : descriptorSet.ImageSamplers ) {
        ...
        ONYX_ASSERT( descriptorSet.UniformBuffers.find( binding ) == descriptorSet.UniformBuffers.end(),
                     "Binding is already present!" );
        ONYX_ASSERT( descriptorSet.StorageBuffers.find( binding ) == descriptorSet.StorageBuffers.end(),
                     "Binding is already present!" );
    ...
    for ( auto& [ binding, imageSampler ] : descriptorSet.SeparateTextures ) {
        ...
        ONYX_ASSERT( descriptorSet.UniformBuffers.find( binding ) == descriptorSet.UniformBuffers.end(),
                     "Binding is already present!" );
        ONYX_ASSERT( descriptorSet.ImageSamplers.find( binding ) == descriptorSet.ImageSamplers.end(),
                     "Binding is already present!" );
        ONYX_ASSERT( descriptorSet.StorageBuffers.find( binding ) == descriptorSet.StorageBuffers.end(),
                     "Binding is already present!" );
    ...
    for ( auto& [ binding, imageSampler ] : descriptorSet.SeparateSamplers ) {
        ...
        ONYX_ASSERT( descriptorSet.StorageBuffers.find( binding ) == descriptorSet.StorageBuffers.end(),
                     "Binding is already present!" );
        ONYX_ASSERT( descriptorSet.SeparateTextures.find( binding ) == descriptorSet.SeparateTextures.end(),
                     "Binding is already present!" );
    ...
    for ( auto& [ bindingAndSet, imageSampler ] : descriptorSet.StorageImages ) {
        ...
        ONYX_ASSERT( descriptorSet.SeparateTextures.find( binding ) == descriptorSet.SeparateTextures.end(),
                     "Binding is already present!" );
        ONYX_ASSERT( descriptorSet.SeparateSamplers.find( binding ) == descriptorSet.SeparateSamplers.end(),
                     "Binding is already present!" );

...
void Pipeline::CreatePipeline( const Shader& shader ) {
    ...
    if( shader.IsComputeShader() ) {
        ...
        const uint32_t colorTargetsCount = static_cast< uint32_t >( colorTargetFormats.size() );
        ...
        const uint32_t vertexAttributesCount = static_cast< uint32_t >( vertexInputs.size() );
        ...
        vertexInputInfo.pVertexAttributeDescriptions = vertexAttributesCount == 0 ? nullptr

...
class InputTypeFactory {
  public:
    ...
    static UniquePtr< T > Create( StringId32 typeId ) { return s_RegisteredTypes[ typeId ].CreateFunctor(); }

    static UniquePtr< T > Copy( const T& other ) { return s_RegisteredTypes[ other.GetTypeId() ].CopyFunctor( other ); }

    static const MetaData& GetMetaData( StringId32 typeId ) {
        ONYX_ASSERT( s_RegisteredTypes.contains( typeId ) );
        return s_RegisteredTypes.at( typeId );
    }

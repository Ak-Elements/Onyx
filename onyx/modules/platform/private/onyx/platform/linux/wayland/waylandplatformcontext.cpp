...
namespace Onyx::Platform::Wayland
{
    ...
    void WaylandPlatformContext::RegisterCallback(void* data, wl_registry* registry, onyxU32 name, const char* interface, onyxU32 version)
    {
        ...
        {
            platformContext.m_Compositor = static_cast<wl_compositor*>(wl_registry_bind(registry, name, &wl_compositor_interface, 3));
        }
        else if (std::strcmp(interface, wl_shm_interface.name) == 0)
        {
            platformContext.m_SharedMemory = static_cast<wl_shm*>(wl_registry_bind(registry, name, &wl_shm_interface, 1));
        }
        else if (std::strcmp(interface, xdg_wm_base_interface.name) == 0)
        {
            platformContext.m_Shell = static_cast<xdg_wm_base*>(wl_registry_bind(registry, name, &xdg_wm_base_interface, 1));
        ...
        {
            platformContext.m_ZxdgDecorationManager = static_cast<zxdg_decoration_manager_v1*>(wl_registry_bind(registry, name, &zxdg_decoration_manager_v1_interface, 1));
        }
        else if (strcmp(interface, zxdg_toplevel_decoration_v1_interface.name) == 0)
        {
            platformContext.m_ZxdgToplevelDecoration = static_cast<zxdg_toplevel_decoration_v1*>(wl_registry_bind(registry, name, &zxdg_toplevel_decoration_v1_interface, 1));
        }

...
Window::Window( uint32_t id, WaylandPlatformContext& context, const WindowSettings& settings )
    : m_Settings( settings )
    , m_id( id )
    , m_Context( &context ) {
    CreateNativeWindow();
...
void Window::CreateNativeWindow() {
    ...
    // static const struct zxdg_toplevel_decoration_v1_listener zxdgTopLevelDecorationListener = {
    // HandleTopLevelDecorationConfigure, };

    static const struct wl_surface_listener wl_surface_listener = { HandleSurfaceEnter, HandleSurfaceLeave };
    ...
    uint8_t* pool_data = reinterpret_cast< uint8_t* >(

...
namespace onyx::game_core::world_transform
{
    ...
    Matrix3<onyxF32> GetScaleMatrix(const TransformComponent& transformComponent)
    {
        ...
        return Matrix3{ column0 * transformComponent.Scale.X, column1 * transformComponent.Scale.Y, column2 * transformComponent.Scale.Z };
    }
    ...
    Matrix4<onyxF32> GetTransform(const TransformComponent& transformComponent)
    {
        ...
        transformMatrix[3] = transformMatrix[0] * transformComponent.Translation[0] + transformMatrix[1] * transformComponent.Translation[1] + transformMatrix[2] * transformComponent.Translation[2] + transformMatrix[3];

        Matrix3<onyxF32> scaleMatrix = GetScaleMatrix(transformComponent);

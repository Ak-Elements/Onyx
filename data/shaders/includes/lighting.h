...
vec3 BRDF( vec3 L, vec3 V, vec3 N, vec3 radiance, vec3 albedo, float metallic, float roughness ) {
    ...
    if( dotNL > 0.0 ) {
        ...
        vec3 spec = D * F * G / max( 0.0001, ( 4.0 * dotNL * dotNV ) );
...
vec3 CalculatePointLights( uint clusterIndex,
                           vec3 V,
                           vec3 N,
                           vec3 worldPosition,
                           vec3 albedo,
    ...
    uint pointLightCount = debugFlag == 2 ? sbo_LightGrid.Cells[ clusterIndex ].PointLightCount
...
vec3 CalculateSpotLights( uint clusterIndex,
                          vec3 V,
                          vec3 N,
                          vec3 worldPosition,
                          vec3 albedo,
    ...
    uint spotLightCount = debugFlag == 2 ? sbo_LightGrid.Cells[ clusterIndex ].SpotLightCount
...
vec3 debug_colors[ 8 ] = vec3[]( vec3( 0, 0, 0 ),
                                 vec3( 0, 0, 1 ),
                                 vec3( 0, 1, 0 ),
                                 vec3( 0, 1, 1 ),
                                 vec3( 1, 0, 0 ),
...
float LinearDepth( float depthSample, float near, float far ) {
    float linear = near * far / ( far + depthSample * ( near - far ) );
    return linear;
}

vec3 CalculatePBRLighting( vec3 worldPosition,
                           vec3 worldNormal,
    ...
    if( debugFlag == 1 ) {
        ...
        uint zTile = uint( max( log2( LinearDepth( fragCoord.z, near, far ) ) * scale + bias, 0.0 ) );
        // cluster view
    ...
    Lo += CalculatePointLights( clusterIndex,
                                V,
                                N,
                                worldPosition,
                                material.Albedo,
                                ...
                                material.Roughness,
                                debugFlag );
    Lo += CalculateSpotLights( clusterIndex,
                               V,
                               N,
                               worldPosition,
                               material.Albedo,
...
uint GetClusterIndex( vec4 fragCoord,
                      uvec3 clusterGridSize,
                      uvec2 clusterSize,
                      float scale,
                      float bias,
                      float near,
                      float far ) {
    float linearDepth = LinearDepth( fragCoord.z, near, far );
    ...
    uint clusterIndex = clusterIndex3D.x + clusterGridSize.x * clusterIndex3D.y +
